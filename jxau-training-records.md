<!-- TOC -->autoauto- [Day1](#day1)auto    - [一、搭建Mongodb环境并建立表存储温度数据](#一搭建mongodb环境并建立表存储温度数据)auto        - [1. 搭建环境：Ubuntu 18.04](#1-搭建环境ubuntu-1804)auto            - [1.1. 安装下载](#11-安装下载)auto            - [1.2. 启动服务](#12-启动服务)auto            - [1.3. 登陆客户端](#13-登陆客户端)auto            - [1.4. 建立demo数据库并在其中添加存有温度信息的表](#14-建立demo数据库并在其中添加存有温度信息的表)auto        - [2. 下载地址](#2-下载地址)auto        - [3. 具体安装流程参考](#3-具体安装流程参考)auto    - [二、搭建三个节点的分布式数据库](#二搭建三个节点的分布式数据库)auto        - [1. 端口分配](#1-端口分配)auto        - [2. 搭建流程](#2-搭建流程)auto            - [2.1. 搭建config集群](#21-搭建config集群)auto        - [3. 测试Mongodb数据库性能](#3-测试mongodb数据库性能)auto            - [3.1. 对刚搭建好的数据库进行测试](#31-对刚搭建好的数据库进行测试)auto- [Day2](#day2)auto    - [一、参考](#一参考)auto        - [1. thinkjs框架](#1-thinkjs框架)auto        - [2. 物流模拟](#2-物流模拟)auto    - [二、后端服务](#二后端服务)auto        - [1. 框架选择](#1-框架选择)auto    - [三、搭建一个后端项目](#三搭建一个后端项目)auto        - [1. Thinkjs](#1-thinkjs)auto            - [1.1. 安装环境](#11-安装环境)auto            - [1.2. 创建项目](#12-创建项目)auto        - [2. flask](#2-flask)auto            - [2.1. 安装下载](#21-安装下载)auto            - [2.2. 启动程序](#22-启动程序)auto    - [四、实现后端项目与mongodb连接](#四实现后端项目与mongodb连接)auto        - [1. Thinkjs](#1-thinkjs-1)auto            - [1.1. 让框架支持mongdb](#11-让框架支持mongdb)auto            - [1.2. 创建mongodb](#12-创建mongodb)auto            - [1.3. 连接mongodb](#13-连接mongodb)auto        - [2. flask](#2-flask-1)auto            - [2.1. 配置环境](#21-配置环境)auto            - [实例化数据库](#实例化数据库)auto    - [五、通过后端实现对mongodb的CURD操作](#五通过后端实现对mongodb的curd操作)auto        - [1. Thinkjs](#1-thinkjs-2)auto            - [1.1. 添加路由](#11-添加路由)auto            - [1.2. 修改初始页面](#12-修改初始页面)auto            - [1.3. thinkjs 对 mongdb 的 CURD](#13-thinkjs-对-mongdb-的-curd)auto            - [1.4. thinkjs 特定 CURD 封装](#14-thinkjs-特定-curd-封装)auto        - [2. flask](#2-flask-2)auto            - [2.1. 添加路由](#21-添加路由)auto            - [2.2. flask 对 mongo 的 CURD](#22-flask-对-mongo-的-curd)auto    - [六、使用小程序获取地理位置的Api](#六使用小程序获取地理位置的api)auto        - [1. 小程序获取位置信息](#1-小程序获取位置信息)auto            - [1.1. 实现流程](#11-实现流程)auto            - [1.2. 实现方法](#12-实现方法)auto        - [2. 小程序调用高德地图api获取地理位置信息](#2-小程序调用高德地图api获取地理位置信息)auto            - [2.1. 实现流程](#21-实现流程)auto            - [2.2. 实现方法](#22-实现方法)auto                - [2.2.1. 获取key](#221-获取key)auto                - [2.2.2. 配置项目](#222-配置项目)auto                - [2.2.3. 调用api 获取数据](#223-调用api-获取数据)auto    - [七、编写地理信息存储Api](#七编写地理信息存储api)auto    - [八、小程序中整合二维码扫描功能](#八小程序中整合二维码扫描功能)auto    - [九、传递数据到后端](#九传递数据到后端)auto        - [1. 生成一个二维码](#1-生成一个二维码)auto    - [十、后端对个人信息数据进行接收，存储到数据库中](#十后端对个人信息数据进行接收存储到数据库中)auto- [Day 3](#day-3)auto    - [一、微信小程序调用地图api](#一微信小程序调用地图api)auto        - [1. 准备工作](#1-准备工作)auto        - [2. 环境配置](#2-环境配置)auto        - [3. 实现方法](#3-实现方法)auto    - [二、百度地图获取数据库表中信息的坐标显示在地图上](#二百度地图获取数据库表中信息的坐标显示在地图上)auto        - [1. index.js代码](#1-indexjs代码)auto        - [2. 获取数据库数据](#2-获取数据库数据)auto    - [三、在小程序中使用echarts组件](#三在小程序中使用echarts组件)auto        - [1. 实现方法](#1-实现方法)auto- [Day 4](#day-4)auto    - [一、设计二维码](#一设计二维码)auto        - [1. 参阅](#1-参阅)auto    - [二、获取位置信息](#二获取位置信息)auto        - [1. 目标](#1-目标)auto            - [1.1. 完成对地图接口的调用，返回相关的调用结果](#11-完成对地图接口的调用返回相关的调用结果)auto        - [2. 实现介绍](#2-实现介绍)auto    - [三、扫描获取二维码中的信息](#三扫描获取二维码中的信息)auto        - [1. 目标](#1-目标-1)auto            - [1.1. 在小程序中使用扫描获取二维码包含的信息](#11-在小程序中使用扫描获取二维码包含的信息)auto            - [1.2. 将获取的二维码信息和位置信息发送给后端](#12-将获取的二维码信息和位置信息发送给后端)auto        - [2. 实现介绍](#2-实现介绍-1)auto    - [四、后端接收保存数据](#四后端接收保存数据)auto        - [1. 目标](#1-目标-2)auto            - [1.1. 接收前端发送的数据请求](#11-接收前端发送的数据请求)auto            - [1.2. 将数据保存在数据库中](#12-将数据保存在数据库中)auto        - [2. 实现介绍](#2-实现介绍-2)auto            - [2.1. 创建相应数据库（mongoDB）](#21-创建相应数据库mongodb)auto            - [2.2. 设计接口（后端使用 ThinkJS 实现）](#22-设计接口后端使用-thinkjs-实现)auto            - [2.3.效果图（数据库中前端发送的数据）](#23效果图数据库中前端发送的数据)autoauto<!-- /TOC -->
